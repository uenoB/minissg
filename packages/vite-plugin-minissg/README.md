...
* [Advanced Features](#advanced-features)
  ...
  * [Obtaining Renderer for a Module](#obtaining-renderer-for-a-module)
...
In addition, the following breaking change has been made:
* The `virtual:minissg/self` feature has been removed.
...
Client-side code can refer to such data by importing itself with
the `?client` query.
...
In client-side code, import `./foo.js?client` in `foo.js`
...
### Obtaining Renderer for a Module

Sometimes it is convenient if the renderer associated to a module can be
obtained.
A special query `?renderer` provides it.
The default export of `./foo.jsx?renderer` is the rendering function of
`foo.jsx`.
...
The following is an example `pages.jsx` of JSX file that aggregates MDX files:
